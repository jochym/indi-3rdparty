...
12. **Sync command resulting in unexpected RA/Dec:** Issuing a `Sync` command with specific RA/Dec results in the driver reporting significantly different values immediately after, even if the property state transitions to `Ok`. This is due to the driver using the system OS clock instead of the `TIME_UTC` property for celestial transformations.
13. **EQUATORIAL_EOD_COORD transitions to Idle after Slew:** Upon completion of a fast GOTO, the `EQUATORIAL_EOD_COORD` property state transitions from `Busy` to `Idle` instead of `Ok`.
